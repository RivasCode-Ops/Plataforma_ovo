# Prova local - sobe API e valida endpoints (sem Docker)
...
try {
  ...
  $pedidoBody = @{
    cliente = @{ nome = "Cliente Teste"; telefone = "89999990000"; endereco = "Rua da Granja, 1" }
  ...
  Write-Host ""
  Write-Host "Prova API concluida. Suba o painel:" -ForegroundColor Yellow
  Write-Host "  npm run iniciar" -ForegroundColor White
  Write-Host "  http://localhost:5173  (admin / plataforma123)" -ForegroundColor White

  $provaFile = Join-Path $root "docs\PROVA_LOCAL_RESULTADO.txt"
  $linhas = @(
    "Prova local: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",
  ...
  $linhas | Set-Content -Path $provaFile -Encoding UTF8
  Write-Host "Resultado em docs/PROVA_LOCAL_RESULTADO.txt" -ForegroundColor Cyan
}
catch {
